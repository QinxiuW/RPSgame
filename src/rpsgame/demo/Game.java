...
public class Game {
  ...
  private void startPlays(int iteration) {
    ...
    for (int x = 0; x < iteration; x++) {
      Play play = new Play(x + 1, this.p1, this.p2);
      this.plays[x] = play;
    }
  }
  ...
  public String toString() {
    String msg = "";
    //Arrays.stream(this.plays).forEach((p) -> concatStrings(msg, p.toString()));
    for(int x = 0; x<this.plays.length;x++){
      msg = msg.concat(this.plays[x].toString());
    }
    return msg;
  }

  private void concatStrings(String main, String toConcat) {
    main = main.concat(toConcat);
  }
